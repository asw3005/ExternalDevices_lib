...
SPI_HandleTypeDef* ADS867x_SPI = &hspi2;
...
void ADS867x_RST(uint8_t state) {
	...
	if (state > 0) {
		ADS867x_RST_ADC_GPIO_Port->BSRR = ADS867x_RST_ADC_Pin;
	}
	else {
		ADS867x_RST_ADC_GPIO_Port->BSRR = ADS867x_RST_ADC_Pin << 16;
	}	
	HAL_Delay(5);
}
...
float ADS867x_GetVoltage(float scale) {
	...
	return  /* ADS867x_INPUT_RANGE * */ scale * (uint16_t)(ADS867x_ReadADC(&ads8671).DataWord >> 16);
}
...
void ADS867x_SPI_Tx(uint8_t *pData, uint8_t size) {
	...
	ADS867x_SPI_CS(ADS867x_CS_ADC_GPIO_Port, ADS867x_CS_ADC_Pin, 0);
	HAL_SPI_Transmit(ADS867x_SPI, pData, size, 10);
	ADS867x_SPI_CS(ADS867x_CS_ADC_GPIO_Port, ADS867x_CS_ADC_Pin, 1);
}
...
void ADS867x_SPI_Rx(uint8_t *pData, uint8_t size) {
	...
	ADS867x_SPI_CS(ADS867x_CS_ADC_GPIO_Port, ADS867x_CS_ADC_Pin, 0);
	HAL_SPI_Receive(ADS867x_SPI, pData, size, 10);
	ADS867x_SPI_CS(ADS867x_CS_ADC_GPIO_Port, ADS867x_CS_ADC_Pin, 1);
}
