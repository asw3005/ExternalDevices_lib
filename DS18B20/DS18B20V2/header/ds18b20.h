...
///Amount of the devices on the bus.
#define DS18B20_AMOUNT_OF_PARTISIPATING_DEVICES 3
...
/*
 * @brief DS18B20 search rom const.
 *
 **/
typedef enum
{
	DS18B20_DISCREPANCY_BIT = 0,
	DS18B20_ONLY_ZEROS_BITS,
	DS18B20_ONLY_FIRSTS_BITS,
	DS18B20_NO_DEVICE
	
} DS18B20_SearchRomBits;
...
/*
 * @brief DS18B20 function command set.
 *
 **/
typedef enum
{
...
/*
 * @brief Reset 1-Wire bus and detected presence pulse.
 *
 * @return  
 *
 **/
typedef uint8_t (*onewire_reset)(void);
 ...
 *
 **/
typedef void (*strong_pullup)(uint8_t state);
 ...
 *
 **/
typedef uint8_t (*onewire_txrx_data_fptr)(uint8_t *buffer, uint16_t size);
...
/*
 * @brief Converted data of the sensor.
 *
 **/
typedef struct __attribute__((aligned(1), packed))
{
	...
	///Power supply type.
	///If it is logic one level(0xFE), power supply mode is normal, otherwise parasite mode.
...
typedef struct
{
	...
	uint8_t Reserved_0; /* Default 0xFF */
	uint8_t Reserved_1;
	uint8_t Reserved_2; /* Default 0x10 */
	///CRC = X8 + X5 + X4 + 1
	uint8_t Crc;
...
typedef struct
{
	...
	delay_fptr delay;
	onewire_reset reset_bus;
	strong_pullup strong_pull_up;
	onewire_txrx_data_fptr tx_data;
	onewire_txrx_data_fptr rx_data;
...
//int8_t I2C_TxData(uint8_t *buffer, uint16_t size);
//int8_t I2C_RxData(uint8_t *buffer, uint16_t size);
